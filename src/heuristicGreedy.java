...
public class heuristicGreedy {
	...
	heuristicGreedy(ArrayList<User> input){
	...
	public void findSetCover() {
		...
		int intervaluser12 = 0;
		while(goal.size()!=k) {
			if(intervaluser12%2==0) {
				boolean isExist = false;
				for(int i=0;i<goal.size();i++) {
					if(goal.get(i).id==greedyPresentUser.get(totalrank.get(intervaluser12/2).user1).id) {
						isExist = true;
					}
				}
				if(!isExist) {
					goal.add(greedyPresentUser.get(totalrank.get(intervaluser12/2).user1));
				}
				
			}else {
				boolean isExist = false;
				for(int i=0;i<goal.size();i++) {
					if(goal.get(i).id==greedyPresentUser.get(totalrank.get(intervaluser12/2).user2).id) {
						isExist = true;
					}
				}
				if(!isExist) {
					goal.add(greedyPresentUser.get(totalrank.get(intervaluser12/2).user2));
				}
			}
			intervaluser12++;
		}
